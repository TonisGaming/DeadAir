...
namespace DeadAir
{
    internal class DAFunctionalities
    {
        ...
        {
            var thisGearItem = DAFunctionalities.canisterItem;
            GearItem canister = GameManager.GetInventoryComponent().GearInInventoryAtCondition("GEAR_Canister", 1 , 0);
            GearItem canisterimprov = GameManager.GetInventoryComponent().GearInInventoryAtCondition("GEAR_ImprovisedFilter", 1, 0);
            ...
            else if (thisGearItem.name == "GEAR_ImprovisedFilter")
            {
                if (charcoal.m_StackableItem.m_Units < 7 && cloth.m_StackableItem.m_Units < 2)
                {
                    HUDMessage.AddMessage("This action requires 7 charcoal and 2 cloth");
                    GameAudioManager.PlayGUIError();
                    return;
                }
                if (charcoal.m_StackableItem.m_Units >= 7 && cloth.m_StackableItem.m_Units >= 2)
                {
                    ...
                    InterfaceManager.GetPanel<Panel_GenericProgressBar>().Launch(("Refilling..."), 2f, 0f, 0f,
                                    "PLAY_CRAFTINGGENERIC", null, false, true, new System.Action<bool, bool, float>(OnCanisterRefillFinished));
                    GearItem.Destroy(thisGearItem);
                    GameManager.GetInventoryComponent().RemoveGearFromInventory(charcoal.name, 7);
                    GameManager.GetInventoryComponent().RemoveGearFromInventory(cloth.name, 2);
        ...
        private static void OnCanisterRefillFinished(bool success, bool playerCancel, float progress)
        {
            if (isImprov == false)
            {
                GameManager.GetPlayerManagerComponent().InstantiateItemInPlayerInventory(DeadAirUtils.canister, 1);
            }
            else
            {
                GameManager.GetPlayerManagerComponent().InstantiateItemInPlayerInventory(DeadAirUtils.canisterimprov, 1);
            }
        }
